...
# VISTA 2: ANTECEDENTES DE LA IA
    ...
    try:
        ...
        st.error("Asegúrate de subir el archivo 'linea_del_tiempo.jpg' a tu repositorio de GitHub.")

# VISTA 3: IA DÉBIL O ESTRECHA
elif opcion == "IA débil o estrecha":
    st.title("IA débil o estrecha")
    archivo_pdf = "ia_debil.pdf"
    try:
        with open(archivo_pdf, "rb") as f:
            pdf_bytes = f.read()
        st.download_button(label="Descargar PDF", data=pdf_bytes, file_name=archivo_pdf, mime="application/pdf")
        st.components.v1.html(f'<iframe src="data:application/pdf;base64,{pdf_bytes.hex()}" width="100%" height="800px"></iframe>', height=800)
    except FileNotFoundError:
        st.error(f"Asegúrate de subir el archivo '{archivo_pdf}' a tu repositorio de GitHub.")

# VISTA 4: IA FUERTE O GENERAL
elif opcion == "IA fuerte o general":
    st.title("IA fuerte o general")
    archivo_pdf = "ia_fuerte.pdf"
    try:
        with open(archivo_pdf, "rb") as f:
            pdf_bytes = f.read()
        st.download_button(label="Descargar PDF", data=pdf_bytes, file_name=archivo_pdf, mime="application/pdf")
...
# VISTA 5: IA SUPERINTELIGENTE
    ...
    archivo_pdf = "ia_superinteligente.pdf"
    try:
        with open(archivo_pdf, "rb") as f:
            pdf_bytes = f.read()
        st.download_button(label="Descargar PDF", data=pdf_bytes, file_name=archivo_pdf, mime="application/pdf")
